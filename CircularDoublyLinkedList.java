...
//TODO
protected void switchPos(Node<E> prev , Node<E> next){ // 
}

protected void addFreq(E element){
  Node<E> targetNode = getTargetNode(element);
  targetNode.frequency++;
  E element2 = targetNode.getPrev().getElement();
  // is new added words frequency is higher or lower?
  if(targetNode.getPrev().frequency == targetNode.frequency){

    if(element.compareTo(element2) < 0 ){
      //  switchPos(targetNode,targetNode.getPrev());
    }
